...
void loop() {
  ...
  // Read analog value from MQ135 sensor every 5 seconds
  if (millis() - lastGasTime >= 5000) {
    ...
    lastGasTime = millis(); // Update the last gas reading time
    // Publish gas value to MQTT
    publish_gas_vibration(gasValue, digitalRead(VIBRATION_PIN));
  }

  // Read digital value from vibration sensor every 5 second
  if (millis() - lastVibrationTime >= 5000) {
    ...
    lastVibrationTime = millis(); // Update the last vibration reading time
  }
